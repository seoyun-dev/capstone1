...
from products.models  import Product, ProductImage, WorkoutLink
...
with open('workout_links.csv') as in_file:
        data_reader = csv.reader(in_file)
        next(data_reader, None) # 출력시 함께 출력되는 맨첫줄을 제외하고 출력하기 위함
        for row in data_reader:
            thumbnail_url = row[0]
            ...
            WorkoutLink.objects.create(thumbnail_url=thumbnail_url, video_url=video_url, product_id=product_id)